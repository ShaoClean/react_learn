...
function BigImg() {
	...
	function getImgIds () {
		cacheDataRef.current.forEach(item=> {
			if (!imgIdsRef.current?.includes(item.id)) imgIdsRef.current.push(item.id)
		})
	}
	...
	return (
        <div>
			<div>
				<button onClick={()=> setPageData({...pageData,pageNumber: --pageData.pageNumber})}>
					上一页
				</button>
				<button onClick={()=> setPageData({...pageData,pageNumber: ++pageData.pageNumber})}>
					下一页
			...
			<div>
				{
					data.length ? data.map((item,index:number)=>{
						return  item.base64_1 ? <img width={30} key={index} src={item.base64_1.replace('\'','')}/> : <Spin />
					}) : <Spin />
				}
			</div>
